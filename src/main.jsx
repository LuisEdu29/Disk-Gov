...
const guideItems = [
  {
    ...
    icon: Flame,
    tone: "red",
    emergency: "Bombeiros — 193",
    doNow: ["Afaste a pessoa da fonte de calor.", "Resfrie a área com água corrente em temperatura ambiente por cerca de 20 minutos.", "Retire anéis, relógios e outros objetos próximos à área, se não estiverem grudados na pele."],
    dont: ["Não coloque gelo, pasta de dente, manteiga ou outros produtos sobre a queimadura.", "Não estoure bolhas nem arranque pele queimada."],
    seek: "Procure atendimento se a queimadura for extensa, profunda ou atingir rosto, mãos, pés, articulações ou regiões íntimas."
  },
  {
    id: "energia",
    ...
    icon: Home,
    tone: "yellow",
    emergency: "Emergência elétrica",
    doNow: ["Confira se apenas sua residência ficou sem energia ou se a interrupção atingiu a região.", "Desligue aparelhos eletrônicos sensíveis da tomada para protegê-los quando a energia voltar.", "Use lanternas em vez de velas sempre que possível."],
    dont: ["Não mexa em fios caídos ou instalações danificadas.", "Não tente reparar a rede elétrica sem conhecimento técnico."],
    seek: "Se houver fio rompido, faíscas, cheiro de queimado ou risco de incêndio, afaste-se e acione o serviço responsável."
  },
  {
    id: "gas",
    ...
    tone: "orange",
    emergency: "Bombeiros — 193",
    doNow: ["Se for seguro, feche o registro do gás.", "Abra portas e janelas para ventilar o ambiente.", "Saia do local e mantenha outras pessoas afastadas."],
    dont: ["Não acenda fósforos, isqueiros ou luzes.", "Não ligue nem desligue aparelhos elétricos dentro do ambiente com cheiro de gás."],
    seek: "Se o cheiro for forte, houver suspeita de vazamento ou qualquer risco de incêndio, saia imediatamente e chame os Bombeiros."
  },
  {
    id: "acidente",
    ...
    doNow: ["Mantenha distância do trânsito e sinalize o local se puder fazer isso com segurança.", "Observe se há feridos e informe a localização exata ao pedir ajuda.", "Siga as orientações do atendente do serviço de emergência."],
    dont: ["Não mova uma vítima ferida sem necessidade, salvo se houver risco imediato no local.", "Não ofereça comida ou bebida a uma pessoa ferida."],
    seek: "Em caso de feridos, perda de consciência, sangramento intenso ou risco no local, acione imediatamente o atendimento de emergência."
  },
  {
    id: "alagamento",
    title: "Alagamento",
    icon: Siren,
    tone: "purple",
    emergency: "Defesa Civil — 199",
    doNow: ["Vá para um local alto e seguro.", "Evite áreas alagadas e acompanhe os alertas das autoridades.", "Se estiver em veículo, abandone-o se houver risco e puder sair com segurança."],
    dont: ["Não atravesse correntezas a pé ou de carro.", "Não toque em fios elétricos ou equipamentos que possam estar energizados."],
    seek: "Em risco de deslizamento, enchente, desabamento ou isolamento de pessoas, acione a Defesa Civil ou o serviço de emergência adequado."
  }
];
        ...
        <AnimatePresence mode="wait">
          ...
          {active === "guia" && (
            <motion.section key="guide" initial={{opacity:0,x:15}} animate={{opacity:1,x:0}} className="content simple-page guide-page">
              <p className="eyebrow">ORIENTAÇÃO RÁPIDA</p>
              <h1>Guia: o que fazer?</h1>
              <p className="muted-text">Passos simples para situações comuns. Em uma emergência, priorize sua segurança e procure ajuda profissional.</p>
              <div className="guide-list">
                {guideItems.map(item => {
                  const Icon = item.icon;
                  return (
                    <details className="guide-card" key={item.id}>
                      <summary><span className={`guide-icon ${item.tone}`}><Icon size={21}/></span><span><strong>{item.title}</strong><small>{item.emergency}</small></span><ChevronRight size={19} className="guide-chevron"/></summary>
                      <div className="guide-content">
                        <h3>Faça agora</h3><ul>{item.doNow.map((x,i)=><li key={i}>{x}</li>)}</ul>
                        <h3>Não faça</h3><ul>{item.dont.map((x,i)=><li key={i}>{x}</li>)}</ul>
                        <h3>Quando procurar ajuda</h3><p>{item.seek}</p>
                      </div>
                    </details>
                  );
                })}
              </div>
              <div className="about-card guide-warning"><ShieldAlert size={20}/><div><strong>Em caso de emergência</strong><span>Use o botão SOS para encontrar rapidamente Polícia, SAMU ou Bombeiros.</span></div></div>
            </motion.section>
          )}
